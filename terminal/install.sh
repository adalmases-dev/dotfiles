...
BIN_PATH="/usr/local/bin/alacritty" # Install will be placed here

if ! has_command "alacritty" || is_update_required "$(alacritty --version | head -n 1 | awk '{print $2}')" "$LATEST_ALACRITTY"; then
  info "Installing/Updating alacritty to $LATEST_ALACRITTY..."
  ...
  cd "$TEMP_DIR"
  git clone --depth 1 --branch "$LATEST_TAG" https://github.com/alacritty/alacritty.git

  cargo build --release --manifest-path alacritty/Cargo.toml
  sudo install -Dm755 alacritty/target/release/alacritty "$BIN_PATH"
...
ln -sfn "$BIN_PATH" "$HOME/.local/bin/x-terminal-emulator"
...
if ! has_command "starship" || is_update_required "$(starship --version | head -n 1 | awk '{print $2}')" "$LATEST_STARSHIP"; then
  info "Installing/Updating starship to $LATEST_STARSHIP."
...
if ! has_command "fzf" || is_update_required "$(fzf --version | awk '{print $1}')" "$LATEST_FZF"; then
  info "Installing/Updating fzf to $LATEST_FZF."
  rm -rf "$HOME/.fzf"
...
if ! has_command "zoxide" || is_update_required "$(zoxide --version | awk '{print $2}')" "$LATEST_ZOXIDE"; then
  info "Installing/Updating zoxide to $LATEST_ZOXIDE."
  cd "$TEMP_DIR"
...
cd "$TEMP_DIR" # Ensure cargo-related temp files don't clutter current dir
...
install_rust_tool() {
  ...
  if ! has_command "$binary" || is_update_required "$($binary --version | head -n1 | awk '{print $2}')" "$latest_tag"; then
    info "Installing/Updating $tool_name to $latest_tag."
...
for dir in "$SCRIPT_DIR"/*; do
  if [[ -d "$dir" ]]; then
    ...
    if [[ $dir_name == "starship" ]]; then # special case: starship
